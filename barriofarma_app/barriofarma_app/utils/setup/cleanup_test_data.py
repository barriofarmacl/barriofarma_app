...
def cleanup_test_data():
    """
    Limpiar todos los datos de prueba residuales, incluyendo:
    - Datos de prueba (TEST-*)
    - Datos UAT (UAT-*)
    - Documentos relacionados (Sales Invoices, POS Invoices, Payment Entries, etc.)
    """
    frappe.set_user("Administrator")
    
    print("=== Limpiando datos de prueba residuales ===\n")
    
    # Limpiar documentos de venta primero (dependen de items, customers, etc.)
    print("Limpiando Sales Invoices de prueba/UAT...")
    sales_invoices = frappe.db.sql("""
        SELECT name, customer, docstatus
        FROM `tabSales Invoice`
        WHERE customer LIKE 'UAT-%'
           OR customer LIKE 'TEST-%'
           OR name LIKE 'ACC-SINV-%'
    """, as_dict=True)
    for si in sales_invoices:
        try:
            ...
            print(f"  ✓ Eliminado: {si.name}")
        except Exception as e:
            # Si falla por referencias rotas, intentar eliminación directa
            try:
                # Primero cancelar si está submitted
                if si.docstatus == 1:
                    frappe.db.sql("UPDATE `tabSales Invoice` SET docstatus = 0 WHERE name = %s", (si.name,))
                # Eliminar child tables primero
                frappe.db.sql("DELETE FROM `tabSales Invoice Item` WHERE parent = %s", (si.name,))
                frappe.db.sql("DELETE FROM `tabSales Invoice Payment` WHERE parent = %s", (si.name,))
                frappe.db.sql("DELETE FROM `tabSales Invoice` WHERE name = %s", (si.name,))
                print(f"  ✓ Eliminado directamente (BD): {si.name}")
            except Exception as e2:
                print(f"  ✗ Error crítico eliminando {si.name}: {e2}")
    
    print("\nLimpiando POS Invoices de prueba/UAT...")
    pos_invoices = frappe.db.sql("""
        ...
        WHERE customer LIKE 'UAT-%'
           OR customer LIKE 'TEST-%'
    """, as_dict=True)
    for pos in pos_invoices:
        try:
            pos_doc = frappe.get_doc("POS Invoice", pos.name)
            if pos_doc.docstatus == 1:
                pos_doc.cancel()
            frappe.delete_doc("POS Invoice", pos.name, force=True, ignore_permissions=True)
            print(f"  ✓ Eliminado: {pos.name}")
        except Exception as e:
            print(f"  ✗ Error eliminando {pos.name}: {e}")
    
    print("\nLimpiando Payment Entries de prueba/UAT...")
    payment_entries = frappe.db.sql("""
        SELECT name, party, docstatus
        FROM `tabPayment Entry`
        WHERE party LIKE 'UAT-%'
           OR party LIKE 'TEST-%'
           OR name LIKE 'ACC-PE-%'
    """, as_dict=True)
    for pe in payment_entries:
        try:
            ...
            print(f"  ✓ Eliminado: {pe.name}")
        except Exception as e:
            print(f"  ✗ Error eliminando {pe.name}: {e}")
    
    print("\nLimpiando Journal Entries de prueba/UAT...")
    journal_entries = frappe.db.sql("""
        SELECT name, docstatus
        FROM `tabJournal Entry`
        WHERE name LIKE 'ACC-JV-%'
    ...
    for je in journal_entries:
        try:
            ...
            print(f"  ✓ Eliminado: {je.name}")
        except Exception as e:
            print(f"  ✗ Error eliminando {je.name}: {e}")
    
    print("\nLimpiando Shelf Movements de prueba/UAT...")
    shelf_movements = frappe.db.sql("""
        SELECT name, item, docstatus
        FROM `tabShelf Movement`
        WHERE item LIKE 'UAT-%'
           OR item LIKE 'TEST-%'
           OR reference_name LIKE 'ACC-SINV-%'
           OR reference_name LIKE 'ACC-POS-%'
    """, as_dict=True)
    for sm in shelf_movements:
        try:
            sm_doc = frappe.get_doc("Shelf Movement", sm.name)
            if sm_doc.docstatus == 1:
                sm_doc.cancel()
            frappe.delete_doc("Shelf Movement", sm.name, force=True, ignore_permissions=True)
            print(f"  ✓ Eliminado: {sm.name}")
        except Exception as e:
            print(f"  ✗ Error eliminando {sm.name}: {e}")
    
    print("\nLimpiando Stock Entries de prueba/UAT...")
    stock_entries = frappe.db.sql("""
        SELECT name, docstatus, from_warehouse, to_warehouse
        FROM `tabStock Entry`
        WHERE name LIKE 'MAT-STE-%'
           OR (from_warehouse LIKE 'UAT-%' OR to_warehouse LIKE 'UAT-%')
           OR (from_warehouse LIKE 'TEST-%' OR to_warehouse LIKE 'TEST-%')
    """, as_dict=True)
    for se in stock_entries:
        try:
            ...
            print(f"  ✓ Eliminado: {se.name}")
        except Exception as e:
            # Si falla por referencias rotas (warehouses que no existen), eliminar directamente
            try:
                # Primero cancelar si está submitted
                if se.docstatus == 1:
                    frappe.db.sql("UPDATE `tabStock Entry` SET docstatus = 0 WHERE name = %s", (se.name,))
                # Eliminar child tables primero
                ...
                print(f"  ✓ Eliminado directamente (BD): {se.name}")
            except Exception as e2:
                print(f"  ✗ Error crítico eliminando {se.name}: {e2}")
    
    print("\nLimpiando Customers de prueba/UAT...")
    customers = frappe.get_all("Customer", 
        filters={"customer_name": ("like", "UAT-%")}, 
    ...
    for customer in customers:
        try:
            ...
            print(f"  ✓ Eliminado: {customer.customer_name}")
        except Exception as e:
            print(f"  ✗ Error eliminando {customer.customer_name}: {e}")
    
    print("\nLimpiando Patients de prueba/UAT...")
    patients = frappe.db.sql("""
        SELECT name, patient_name
        FROM `tabPatient`
        WHERE patient_name LIKE 'Paciente Test%'
           OR patient_name LIKE 'UAT-%'
           OR patient_name LIKE 'TEST-%'
    """, as_dict=True)
    for patient in patients:
        try:
            frappe.delete_doc("Patient", patient.name, force=True, ignore_permissions=True)
            print(f"  ✓ Eliminado: {patient.patient_name}")
        except Exception as e:
            print(f"  ✗ Error eliminando {patient.patient_name}: {e}")
    ...
    # Limpiar Batches de prueba/UAT
    print("\nLimpiando Batches...")
    batches = frappe.get_all("Batch", filters={"batch_id": ("like", "BATCH-%")}, fields=["name", "batch_id"])
    batches.extend(frappe.get_all("Batch", filters={"batch_id": ("like", "UAT-%")}, fields=["name", "batch_id"]))
    batches.extend(frappe.get_all("Batch", filters={"batch_id": ("like", "TEST-%")}, fields=["name", "batch_id"]))
    ...
    # Limpiar Warehouses de prueba/UAT
    ...
    # Buscar por nombre o por código que empiece con TEST-WH o UAT-WH
    warehouses = frappe.db.sql("""
        ...
        WHERE name LIKE 'TEST-WH%' 
           ...
           OR warehouse_name LIKE 'UAT-WH%'
           OR warehouse_name LIKE 'TEST-%'
           OR warehouse_name LIKE 'UAT-%'
    """, as_dict=True)
    ...
    # Limpiar Items de prueba/UAT
    print("\nLimpiando Items...")
    items = frappe.get_all("Item", filters={"item_code": ("like", "TEST-ITEM-%")}, fields=["name", "item_code"])
    items.extend(frappe.get_all("Item", filters={"item_code": ("like", "UAT-%")}, fields=["name", "item_code"]))
    for item in items:
        try:
    ...
    for presc in prescriptions:
        try:
            frappe.delete_doc("Receta Medica", presc.name, force=True, ignore_permissions=True)
...
def cleanup_test_users():
    ...
    - Usuarios UAT (farmaceutico.uat@barriofarma.cl, auxiliar.uat@barriofarma.cl, etc.)
    - Usuarios con emails @example.com (excepto Administrator y Guest)
    
    Uso:
        bench --site barriofarma.localhost execute \
            barriofarma_app.barriofarma_app.utils.setup.cleanup_test_data.cleanup_test_users
    """
    frappe.set_user("Administrator")
    ...
    # 3. Buscar usuarios UAT específicos
    uat_users = frappe.db.sql("""
        SELECT name, email, full_name
        FROM `tabUser`
        WHERE email LIKE '%uat@barriofarma.cl%'
           OR email LIKE '%UAT%'
           OR name LIKE '%uat%'
    """, as_dict=True)
    
    # 4. Buscar usuarios con email @example.com (excluyendo Administrator y Guest)
    example_email_users = frappe.get_all("User",
        filters=[
            ["email", "like", "%@example.com"],
            ["name", "not in", ["Administrator", "Guest", "administrator", "guest"]]
        ],
        fields=["name", "email", "full_name"]
    )
    ...
    all_test_users = {}
    for user in frappe_test_users + barriofarma_test_users + uat_users + example_email_users:
...
def check_test_users():
    ...
    Uso:
        bench --site barriofarma.localhost execute \
            barriofarma_app.barriofarma_app.utils.setup.cleanup_test_data.check_test_users
    """
    print("\n" + "=" * 70)
...
def cleanup_all():
    ...
    Uso:
        bench --site barriofarma.localhost execute \
            barriofarma_app.barriofarma_app.utils.setup.cleanup_test_data.cleanup_all
    """
    print("\n" + "=" * 70)
